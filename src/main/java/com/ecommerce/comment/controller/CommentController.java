...
public class CommentController {
    ...
    // Count approved comments : isApproved = true
    @Operation(summary = "Count approved comments",
            description = "Retrieve the total number of approved comments.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved the count of approved comments"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @GetMapping("/count-comments")
    public ResponseEntity<?> countApprovedComments() {
        Long count = commentService.countApprovedComments();
        return ResponseEntity.ok(new SuccessResponse<>("Count successfully", count));
    }
